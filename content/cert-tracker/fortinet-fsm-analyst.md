...
exam_title: "Fortinet NSE 6 - FortiSIEM 7.4 Analyst"
exam_level: "professional"
exam_status: "active"
exam_category: "Fortinet"
vendor: "fortinet"
manual: false
guided_slug: "fortinet-fsm-analyst"
---
...
1. **Think like an analyst, not an architect** — for every task, ask "how do I *find, correlate, or act on* this in FortiSIEM?" (search + rules + incidents), not "how do I design the deployment?"
2. **Use our practice exam** — try the 20 free questions first to gauge your readiness
3. **Master Analytics search** — filter operators, group-by and aggregation functions, CMDB and lookup-table queries, and nested query lookups; know real-time vs historical search and how retention/archive affects results
4. **Learn the rule engine** — rule components, subpatterns (filter + aggregation + group-by + threshold/time-window), FOLLOWED-BY / NOT-FOLLOWED-BY relationships, clearing conditions, watch lists, and MITRE ATT&CK mapping
5. **Separate detection from response** — notification policies and channels vs remediation scripts that act through FortiGate and FortiEDR — and know where FortiEDR policies, FCS, ML/UEBA anomalies and ZTNA telemetry fit into the analyst workflow
6. **Simulate exam conditions** — use the timed exam mode to practice reasoning about FortiSIEM search logic and rule subpatterns under pressure
