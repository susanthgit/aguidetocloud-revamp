---
title: "DVA-C02: AWS Developer Associate — Free Study Guide"
description: "DVA-C02: the AWS Developer Associate exam. Free 200-question practice exam + study guide covering SDKs, deployment patterns, and application development."
type: "cert-tracker"
layout: "single"
exam_code: "DVA-C02"
exam_title: "AWS Developer Associate"
exam_level: "intermediate"
exam_status: "active"
exam_category: "AWS"
vendor: "aws"
manual: false
guided_slug: "aws-dva-c02"
---
## About the DVA-C02 Exam

> Build and deploy cloud-native applications on AWS

Master AWS development — Lambda, DynamoDB, API Gateway, S3, Cognito, KMS, CodePipeline, SAM, CDK, X-Ray, and CloudWatch for building, securing, deploying, and troubleshooting cloud-native applications.

## Who Should Take This Exam?

The DVA-C02 is designed for **IT professionals with some hands-on experience**. 6-12 months of hands-on experience recommended.

**Typical study time:** 4-8 weeks of focused study

## Exam Quick Facts

| Detail | Value |
|--------|-------|
| **Exam Code** | DVA-C02 |
| **Title** | AWS Developer Associate |
| **Duration** | 130 minutes |
| **Questions** | 65 |
| **Pass Score** | 720 / 1000 |
| **Cost** | $150 USD |
| **Provider** | Pearson VUE / PSI |
| **Validity** | 3 years |
| **Question Types** | Multiple choice, Multiple response |
| **Official Page** | [View on AWS →](https://aws.amazon.com/certification/certified-developer-associate/) |

## Exam Domains & Weights

The DVA-C02 exam covers **4 domains**. Focus your study time based on the weights below — higher-weighted domains have more exam questions.

| Domain | Weight | Practice Qs |
|--------|--------|-------------|
| Development with AWS Services | 32% | 64 |
| Security | 26% | 52 |
| Deployment | 24% | 48 |
| Troubleshooting and Optimization | 18% | 36 |
| **Total** | **100%** | **200** |

> 💡 **Study tip:** **Development with AWS Services** carries the most weight (32%) — start there. **Troubleshooting and Optimization** has the least (18%), but don't skip it — exam questions can come from any domain.

## Practice Exam — 200 Questions

Prepare for the DVA-C02 with our **200-question practice exam** covering all 4 exam domains. Every question includes detailed explanations and maps to official exam objectives.

**What you get:**
- ✅ Exam simulation mode with timer
- ✅ Spaced repetition for weak areas
- ✅ Detailed explanations for every question
- ✅ Progress tracking across domains
- ✅ 20 free questions — no account needed

## AWS Certification Path

AWS certs follow: Foundational → Associate → Professional, plus Specialty tracks. Cloud Practitioner is the recommended starting point.

## Related AWS Certifications

If you're studying for the DVA-C02, you might also be interested in these AWS certifications:

- **[AIF-C01: AWS AI Practitioner](/cert-tracker/aws-aif-c01/)** — 200 practice questions
- **[DAS-C01: AWS Data Analytics Specialty](/cert-tracker/aws-das-c01/)** — 200 practice questions
- **[DBS-C01: AWS Database Specialty](/cert-tracker/aws-dbs-c01/)** — 200 practice questions
- **[DEA-C01: AWS Data Engineer Associate](/cert-tracker/aws-dea-c01/)** — 200 practice questions
- **[MLA-C01: AWS Machine Learning Engineer Associate](/cert-tracker/aws-mla-c01/)** — 200 practice questions

## Study Tips

1. **Start with the heaviest domain** — focus your time where the exam focuses its questions
2. **Use our practice exam** — try the 20 free questions first to gauge your readiness
3. **Review explanations** — don't just check if you got it right; read why each answer is correct
4. **Simulate exam conditions** — use the timed exam mode to practice under pressure
5. **Check the official page** — [official exam details](https://aws.amazon.com/certification/certified-developer-associate/) always have the latest objectives
