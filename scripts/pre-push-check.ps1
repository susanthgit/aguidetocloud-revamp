...
if (-not $certChanged) {
    ...
    if (Test-Path $certTest) {
        ...
        Write-Host " ❌ BLOCKED" -ForegroundColor Red
        Write-Host "  scripts/lib/cert-description.test.js is missing — the pricing-truthfulness gate cannot run." -ForegroundColor Yellow
        Write-Host "  Restore it before pushing cert-tracker changes." -ForegroundColor Yellow
        $failed = $true
    }
}

# ─── CHECK 12: Cert lifecycle truthfulness ───
# Added 2026-09-14. Five exams had gone GA months earlier but their pages still
# said "in beta" and promised an 80% beta discount, and three genuinely-beta
# exams rendered as "Active" because they were missing from latest.json.
...
if (-not $lifecycleChanged) {
    ...
    $lifecycleScript = Join-Path $PSScriptRoot "check-cert-lifecycle.mjs"
    if (Test-Path $lifecycleScript) {
        $lifecycleOutput = & node $lifecycleScript 2>&1
        if ($LASTEXITCODE -ne 0) {
            Write-Host " ❌ BLOCKED" -ForegroundColor Red
            $lifecycleOutput | Where-Object { $_ -match '✗|problem' } | ForEach-Object { Write-Host "  $_" -ForegroundColor Yellow }
            Write-Host "  An exam that has gone GA must not still be advertised as beta." -ForegroundColor Yellow
            $failed = $true
        } else {
            Write-Host " ✅ Cert statuses agree with page copy" -ForegroundColor Green
        }
    } else {
        Write-Host " ❌ BLOCKED" -ForegroundColor Red
